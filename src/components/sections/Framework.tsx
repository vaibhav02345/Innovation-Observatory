...
const Framework = () => {
  ...
  return (
    <section id="framework" className="relative py-20 lg:py-40 bg-surface/20 border-t border-b border-text-secondary/20 overflow-hidden">
      ...
      <div id="framework-container" className="relative w-full max-w-6xl mx-auto px-4 lg:px-6 py-10 lg:py-20 h-auto md:h-[1000px] lg:h-[1200px]">
        ...
        <div className="absolute top-10 bottom-10 left-[36px] sm:left-[48px] w-1 z-0 pointer-events-none md:hidden bg-text-secondary/10 rounded-full">
           <div ref={mobileProgressRef} className="w-full bg-accent rounded-full h-0"></div>
        </div>
        
        <div className="relative z-10 flex flex-col space-y-12 sm:space-y-16 md:space-y-0 md:block w-full md:h-full">
          {steps.map((step, i) => {
            ...
            const desktopPositions = [
              "md:left-[60%] md:top-[0%]",
              "md:left-[40%] md:top-[25%]",
              "md:left-[60%] md:top-[50%]",
              "md:left-[40%] md:top-[75%]",
              "md:left-[60%] md:top-[100%]"
            ];
            ...
            return (
              <div 
                ...
                className={`flex items-center transition-all w-full md:w-[96px] md:h-[96px] md:absolute md:-translate-x-1/2 md:-translate-y-1/2 ${desktopPositions[i]} z-20`}
              >
                {/* Mobile specific Number Node (hidden on desktop) */}
                <div className="md:hidden node-num text-accent font-mono text-[10px] sm:text-sm font-bold border-2 border-accent rounded-full min-w-[32px] sm:min-w-[40px] h-[32px] sm:h-[40px] flex items-center justify-center bg-background z-20 transition-colors duration-500 shadow-[0_0_15px_rgba(0,0,0,0.5)] mr-4 sm:mr-6">
                  {i+1}
                </div>
                ...
                <div className="flex relative items-center justify-center w-12 h-12 sm:w-16 sm:h-16 lg:w-24 lg:h-24 shrink-0">
                  ...
                  <div className={`hidden md:flex absolute -top-3 ${isTextOnRight ? '-left-3' : '-right-3'} node-num text-accent font-mono text-[10px] lg:text-sm font-bold border-2 border-[#111] rounded-full min-w-[36px] h-[36px] items-center justify-center bg-background z-30 transition-colors duration-500`}>
                    {i+1}
                  </div>
                </div>

                {/* Desktop Text Layout (Absolute positioned) */}
                <div className={`hidden md:flex absolute top-1/2 -translate-y-1/2 w-[280px] lg:w-[400px] ${isTextOnRight ? 'left-full pl-6 lg:pl-10 justify-start' : 'right-full pr-6 lg:pr-10 justify-end'}`}>
                  <div className={`flex flex-col ${isTextOnRight ? 'text-left' : 'text-right'}`}>
                    <span className="text-xl lg:text-3xl font-satoshi font-bold uppercase mb-2 tracking-widest text-text-primary">
                      {step.title}
                    </span>
                    <span className="text-sm lg:text-base text-text-secondary font-inter leading-relaxed">
                      {step.sub}
                    </span>
                  </div>
                </div>
                
                {/* Mobile Text (Standard flex layout) */}
                <div className="flex flex-col md:hidden w-full ml-4 sm:ml-6">
                  <span className="text-[13px] sm:text-xl font-satoshi font-bold uppercase mb-1 sm:mb-2 tracking-widest text-text-primary">
                    {step.title}
                  </span>
                  <span className="text-[10px] sm:text-sm text-text-secondary font-inter leading-relaxed">
                    {step.sub}
                  </span>
                </div>
